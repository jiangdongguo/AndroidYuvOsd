//
// Created by jiangdongguo on 2018/10/19.
//
...
// 16*16点阵字模，每个汉字字模需要16x16/8=32个字节存放
...
// 16*8点阵字模(16行，8列)，每个数字或符号占16*8/8=16个字节
 ...
 * Description:  实现在yuv420图片上面时间(暂时不支持汉字)
...
char *draw_Font_Func(char *ptr_frame, int FRAME_WIDTH, int FRAME_HEIGHT,
                     int startx, int starty, char *str) {
    ...
    offsetU = offsetY + FRAME_WIDTH * FRAME_HEIGHT;     // U分量起始地址，这里暂时用不到
    offsetV = offsetU + FRAME_WIDTH * FRAME_HEIGHT / 4; // V分量起始地址，这里暂时用不到

    // 遍历字符串所有字符，阿拉伯数字0~9的ASCII码为48~57
    // '-'的ASCII码为45
    for (i = 0; i < nStrlen; i++) {
        int temp1 = str[i] - 48;
        if (temp1 == -3) {
        ...
        for (j = 0, y = starty; j < 16 && y < FRAME_HEIGHT - 1; j++, y++) {
            ...
            for (k = 0, x = startx + i * 8; k < 8 && x < FRAME_WIDTH - 1;
                 k++, x++)    // dots in a line
            {
                // 取一个字节里的所有为，为1就上色，为0向右偏移1位以便取下一位
                if (p8 & mask8) {
                    // Y分量为亮度
...
//                    } else {
                    // y * FRAME_WIDTH表示第几个像素，x为Y分量的宽度 1个字节
